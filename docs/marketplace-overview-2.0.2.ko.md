...
2. `보기 > 다른 창 > Raw Buffer Visualizer`를 한 번 엽니다.
3. 디버깅을 시작하고 이미지 변수가 대입된 다음 줄에서 중단합니다.
4. 등록된 이미지와 컬렉션은 DataTip, Locals, Autos, Watch의 시각화 아이콘으로 열고, 초기화된 OpenCvSharp/Emgu Mat 및 호환 애플리케이션 래퍼는 **Auto Inspect on Break**로 열거나 필요할 때 **Scan Now**를 사용합니다.
...
- 지원되는 형식의 목록, 혼합 목록, 딕셔너리 및 1차원 이미지 배열
...
- x64 Community, Professional 및 Enterprise의 Visual Studio 2022 `17.9+` 지원을 복원했습니다.
- 동일한 Marketplace 확장 ID로 안정 버전 Visual Studio 2026 `18.x` 지원을 유지합니다.
- 쓰기 가능한 임시 저장소 경로가 길어도 등록 디버거 이미지가 도킹 뷰어에 전달되도록 보완했습니다.
- Visual Studio 기본 스타일의 도구 모음 아이콘, 반응형 명령 라벨, 명확한 빈 뷰어 안내 및 이미지 상태에 맞는 명령 활성화를 추가했습니다.
- 별도 컬렉션 옵션 없이 지원되는 정확한 OpenCvSharp/Emgu Mat 목록과 1차원 배열을 자동으로 포함합니다.
- **Clear**가 Inspector의 열림/닫힘 상태는 유지하면서 전체 뷰어 표시 상태를 초기화합니다.
...
정확한 `2.0.2` 패키지는 Visual Studio 2022 `17.14.37516.0`과 Visual Studio 2026 `18.8.12023.21`에서 설치 검증을 완료했습니다. Visual Studio 2022 패키지는 Visual Studio `17.9` SDK 하한을 기준으로 빌드하고 패키지 검사를 수행했습니다.
