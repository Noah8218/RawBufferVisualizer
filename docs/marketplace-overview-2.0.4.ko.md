...
여러 디버거 이미지가 쌓이면 이미지 목록 바로 위의 **Clear all**로 한 번에 정리할 수 있습니다. 이 동작은 불러온 이미지를 모두 제거하고 뷰어를 초기화하지만 중단된 디버기 데이터는 변경하지 않습니다. 이미지가 없을 때는 버튼이 비활성화됩니다.
...
- 전체 뷰어 초기화 기능을 이미지 목록 바로 위의 명확한 **Clear all** 동작으로 옮겼습니다.
- 직접 연 20개 이미지 컬렉션과 문서 의존 뷰어·비교·진단·선택 상태를 한 번에 정리합니다.
- 이미지가 없을 때는 버튼을 비활성화하고, 중단된 디버기 데이터는 변경하지 않는다는 점을 도움말에 명시합니다.
- 2.0.3에서 추가된 동시성 딕셔너리 직접 열기와 최초 `ImagePtr` 전달 수정을 그대로 유지합니다.
...
`2.0.4` 패키지는 Visual Studio `17.9` SDK 하한과 `2.0.3`에서 사용한 동일한 인프로세스/아웃오브프로세스 패키지 분리 경계를 유지합니다. 정확한 설치 호스트 검증 결과는 지원 범위와 분리해 기록하며, 해당 패키지가 실제로 통과한 호스트만 검증 완료로 명시합니다.
