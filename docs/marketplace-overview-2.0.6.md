...
- Advances the immutable Marketplace package identity from public `2.0.5.0` to `2.0.6.0`.
- Makes no functional or compatibility change to the validated 2.0.5 behavior.
- Retains exact debugger expression names, native `Ptr` versus `Pixels` provenance, complete image-array registration, fail-closed pointer reads, and negative-stride Bitmap correction.
...
The `2.0.6` package keeps the same Marketplace identity, Visual Studio `17.9` SDK floor, and isolated in-process/out-of-process package boundary. Exact installed-host qualification is recorded separately from the supported range; a host is named as verified only after that exact package has passed there.
