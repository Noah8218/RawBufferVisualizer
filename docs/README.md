...
The current source correction candidate is `2.0.8.0`. `main` was fast-forwarded from `agent/vs2022-17.9-compat`, and the reviewed integration was pushed to `origin/main` through `b11448b`; the following main commit records that completed push without changing product source or the VSIX. Official Gallery API readback on 2026-09-04 confirms public `2.0.7.0`, but that package's `netstandard2.0` debugger ObjectSource is byte-identical to 2.0.6 and fails direct OpenCvSharp `CV_32SC1` visualization on the reported Visual Studio 2022 17.9 debugger host. Reinstalling 2.0.7 is not remediation.
