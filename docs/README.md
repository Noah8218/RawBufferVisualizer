...
Version transition: publish unchanged `1.0.53` as the qualified 1.x stabilization bridge after source commit and explicit Marketplace approval, then start the separate 2.0 development line. Do not relabel the qualified binary as `2.0.0`; a version change creates a different package and requires fresh qualification.
