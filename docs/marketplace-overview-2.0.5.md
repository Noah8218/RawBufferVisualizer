...
Each image row starts with the actual debugger object or expression name, including collection entries such as `imageList[0]`, `imageDictionary[key]`, and `bitmapArray[1]`. When native provenance is available, OpenCvSharp and Emgu rows show the object's `Ptr` separately from the `Pixels` address (`Data` or `DataPointer`) used to read image bytes. ImagePtr and `RawBufferView` combine equal source/pixel addresses as `Ptr / Pixels` or `Buffer / Pixels`. Bitmap records `Scan0 / Pixels` only as captured provenance while `LockBits` is active; it is never described as a live Bitmap pointer.

The row also shows the process ID and `LIVE`, `CAPTURED`, `PREVIEW`, or `UNAVAILABLE` state. Right-click it to copy the full pointer address or pixel address independently. `CAPTURED` means the bytes were copied when the visualizer opened, while `LIVE` can read the current paused process.
...
- Shows the exact debugger object/expression name above each thumbnail, including resolved collection-root names.
- Distinguishes OpenCvSharp/Emgu `Ptr` from the `Pixels` address, combines equal ImagePtr/RawBufferView addresses, and records Bitmap `Scan0 / Pixels` as captured provenance.
- Adds separate **Copy pointer address** and **Copy pixel address** actions and includes both values in Descriptor details.
- Shows process ID plus `LIVE`, `CAPTURED`, `PREVIEW`, or `UNAVAILABLE` state when provenance is available.
- Reopens the same address as a new observation of its current bytes; an address is not treated as an object identity.
- Stops with a controlled error when pointer memory has been released, is inaccessible, or cannot be read completely, instead of displaying incomplete image data.
- Corrects top-to-bottom transfer for `Bitmap` images whose native stride is negative.
- Fixes direct Bitmap/OpenCvSharp/Emgu array registration by using complete assembly-qualified target identities.
- Retains **Clear all**, direct concurrent dictionaries, Visual Studio 2022 `17.9+`, and stable Visual Studio 2026 `18.x`.
...
The `2.0.5` package keeps the same Marketplace identity, Visual Studio `17.9` SDK floor, and isolated in-process/out-of-process package boundary. Exact installed-host qualification is recorded separately from the supported range; a host is named as verified only after that exact package has passed there.
...
- Keep a pointer-backed buffer alive and unchanged until transfer finishes. Released, inaccessible, or partially readable ranges stop with an error instead of producing a partial image.
- A source address is a memory location, not an object identity. Opening the same address again reads its current bytes, which may belong to a different allocation.
