...
3. Start debugging and stop after the image variables have been assigned.
4. Use the DataTip, Locals, Autos, or Watch visualizer icon for registered images and collections; let **Auto Inspect on Break** open initialized OpenCvSharp/Emgu Mats and compatible application wrappers; or use **Scan Now** on demand.
...
- Supported typed or mixed lists, dictionaries, and one-dimensional image arrays
...
- Restores Visual Studio 2022 `17.9+` support for x64 Community, Professional, and Enterprise.
- Retains stable Visual Studio 2026 `18.x` support through the same Marketplace extension identity.
- Prevents long writable temporary-storage paths from blocking registered debugger images from reaching the docked viewer.
- Adds Visual Studio-native toolbar icons, responsive command labels, clear empty-viewer guidance, and image-aware command states.
- Includes supported exact OpenCvSharp/Emgu Mat lists and one-dimensional arrays automatically, without a separate collection option.
- Makes **Clear** reset the complete viewer presentation while preserving whether Inspector is open or closed.
...
The exact `2.0.2` package completed installed verification on Visual Studio 2022 `17.14.37516.0` and Visual Studio 2026 `18.8.12023.21`. Its Visual Studio 2022 package is built and package-checked against the Visual Studio `17.9` SDK floor.
