...
```xml
<Identity Id="RawBufferVisualizer.34f8ad30-2f11-4c37-a9d4-00f3a8c1d29f" Version="2.0.8.0" Language="en-US" Publisher="Noah Choi" />
```
...
powershell -ExecutionPolicy Bypass -File .\scripts\Publish-VisualStudioExtension.ps1 -Configuration Release -Framework net472 -ViewerFramework net472 -BuildRoot D:\OpenVisionLab-TestData\RawBufferVisualizer\release-2.0.8\20260904\build -PublishRoot D:\OpenVisionLab-TestData\RawBufferVisualizer\release-2.0.8\20260904\candidate -NoZip
powershell -STA -ExecutionPolicy Bypass -File .\scripts\SmokeDockedLayoutWidths.ps1 -Configuration Release -Framework net472 -NoBuild -OutputDir D:\OpenVisionLab-TestData\RawBufferVisualizer\release-2.0.8\20260904\layout-widths
...
Use only the exact candidate recorded in [release-qualification-2.0.8.md](release-qualification-2.0.8.md). Do not upload it until the owner approves the candidate and copy, the matching source is committed and pushed, CI passes for that commit, and the exact bytes pass installed qualification on the required available hosts. Do not rebuild or substitute the candidate bytes after qualification.

Validate the exact package and generated Marketplace manifest without publishing:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\Publish-VisualStudioMarketplace.ps1 -VsixPath <FULLY_QUALIFIED_2_0_8_VSIX> -OverviewPath .\docs\marketplace-overview-2.0.8.md -Publisher openvisionlab -PublishManifestPath D:\OpenVisionLab-TestData\RawBufferVisualizer\release-2.0.8\20260904\marketplace\vs-publish.json -DryRun
```

Omitting `-VsixPath` is an error. Passing public `2.0.7` or any earlier/superseded candidate as `2.0.8` is also an error. Before upload, the source must be committed/pushed, CI must pass for that commit, installed qualification must pass, and the exact candidate path, length, and SHA-256 must match [release-qualification-2.0.8.md](release-qualification-2.0.8.md). Marketplace upload remains owner-controlled.

## Pre-publish clean/update gate

Before uploading, perform both a clean install and an in-place update from exact public `2.0.7` on a serviced Visual Studio 2022 profile. Also qualify the same bytes on real VS2022 `17.9` and stable VS2026 `18.x`. Do not uninstall the public baseline, run registration repair, or clear skipped-package state before the update test.
...
Install the same unchanged VSIX on real Visual Studio 2022 `17.9` when available, the current serviced Visual Studio 2022 instance, and stable Visual Studio 2026 `18.x`; repeat at least `Int32Industrial`, `ImagePtrColdStart`, `ConcurrentDictionary`, `ReleaseAnnouncement`, `AutomaticCollections`, `MultiLibraryHybrid`, `EnvironmentCheck`, and the pointer provenance/safety changed path. Pass `-VisualStudioInstanceId` explicitly so installation and smoke evidence name the intended product generation. The `2.0.8` VSIX manifest uses `[17.9,18.0)`; Visual Studio 2026 supports API version 17.x and evaluates only the lower bound for compatibility. Record each exact host build and do not substitute a Preview/Insiders build. Because 2.0.8 corrects a `ClrCustomVisualizerVSHost` payload failure reported on 17.9, exact 17.9 direct `CV_32SC1` DataTip/Locals execution is a required publication gate, not optional compatibility debt.
...
| `categories` | `other` |
| `expected_version` | Exact VSIX version, for example `2.0.8.0`. |
...
- The update from public `2.0.7` to candidate/public `2.0.8` requires uninstall, repair, `/ResetSkipPkgs`, or another recovery action.
- Exact Visual Studio 2022 17.9 direct `CV_32SC1` visualization still reports `Unsupported Mat type: CV_32SC1` or has not been exercised on the corrected bytes.
