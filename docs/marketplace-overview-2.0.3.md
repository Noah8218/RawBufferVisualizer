...
- Opens `ConcurrentDictionary<TKey,TValue>` image collections directly from DataTip, Locals, Autos, and Watch on .NET Framework and current .NET.
- Preserves concurrent dictionary keys as image-row names, reports unsupported values independently, and keeps each invocation bounded to 256 entries.
- Fixes the first registered `ImagePtr` handoff when the docked viewer has not been opened yet; no manual Tool Window open is required before using the visualizer.
...
The `2.0.3` package keeps the Visual Studio `17.9` SDK floor and the same isolated in-process/out-of-process package boundary used by `2.0.2`. The installed candidate passed the new `ConcurrentDictionary` and first-use `ImagePtr` scenarios on Visual Studio 2022 Community `17.14.37516.0` and Visual Studio 2026 Community `18.8.12105.206`. Visual Studio 2022 `17.9` remains covered by the package/SDK floor and earlier unchanged-architecture runtime evidence; the exact `2.0.3` bytes were not run on a currently installed 17.9 host.
