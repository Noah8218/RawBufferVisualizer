...
여러 디버거 이미지가 쌓이면 이미지 목록 바로 위의 **Clear all**로 한 번에 정리할 수 있습니다. 이 동작은 불러온 이미지를 모두 제거하고 뷰어를 초기화하지만 중단된 프로그램의 데이터는 변경하지 않습니다. 이미지가 없을 때는 버튼이 비활성화됩니다.

각 이미지 행의 맨 위에는 실제 디버거 객체 또는 식 이름이 표시됩니다. 컬렉션도 `imageList[0]`, `imageDictionary[key]`, `bitmapArray[1]`처럼 루트 객체명을 유지합니다. 네이티브 출처 정보가 있으면 OpenCvSharp와 Emgu 행은 이미지 바이트를 읽는 `Pixels` 주소(`Data` 또는 `DataPointer`)와 객체의 `Ptr`을 분리해 보여줍니다. ImagePtr와 `RawBufferView`처럼 두 주소가 같으면 `Ptr / Pixels` 또는 `Buffer / Pixels`로 합쳐 표시합니다. Bitmap의 `Scan0 / Pixels`는 `LockBits`가 활성화된 순간에만 얻은 캡처 출처이며 라이브 Bitmap 포인터로 표시하지 않습니다.

같은 행에 프로세스 ID와 `LIVE`, `CAPTURED`, `PREVIEW`, `UNAVAILABLE` 상태도 표시됩니다. 마우스 오른쪽 버튼 메뉴에서 전체 포인터 주소와 픽셀 주소를 각각 복사할 수 있습니다. `CAPTURED`는 시각화할 때 바이트를 복사했다는 의미이며, `LIVE`는 현재 중단된 프로세스를 읽을 수 있다는 의미입니다.
...
- 썸네일 위에 실제 디버거 객체/식 이름을 표시하고 컬렉션 루트 이름도 유지합니다.
- OpenCvSharp/Emgu의 `Ptr`과 `Pixels` 주소를 구분하고, 주소가 같은 ImagePtr/RawBufferView는 합쳐 표시하며, Bitmap `Scan0 / Pixels`는 캡처 출처로 기록합니다.
- 이미지 행 메뉴에 **Copy pointer address**와 **Copy pixel address**를 분리해 제공하고 Descriptor에도 두 값을 표시합니다.
- 출처 정보가 있을 때 프로세스 ID와 `LIVE`, `CAPTURED`, `PREVIEW`, `UNAVAILABLE` 상태를 표시합니다.
- 같은 주소를 다시 열어도 이전 객체로 간주하지 않고 그 시점의 바이트를 새로 읽습니다.
- 포인터 메모리가 해제되었거나 접근할 수 없거나 완전히 읽히지 않으면 불완전한 이미지를 표시하지 않고 제어된 오류로 중단합니다.
- 네이티브 stride가 음수인 `Bitmap`을 위에서 아래 순서로 올바르게 전달합니다.
- Bitmap/OpenCvSharp/Emgu 배열을 완전한 어셈블리 정규화 이름으로 등록해 직접 열 때 발생하던 Visual Studio 버전 문자열 오류를 수정합니다.
- **Clear all**, 동시성 딕셔너리 직접 열기, Visual Studio 2022 `17.9+` 및 안정 버전 Visual Studio 2026 `18.x` 지원을 유지합니다.
...
`2.0.5` 패키지는 동일한 Marketplace ID, Visual Studio `17.9` SDK 하한 및 분리된 인프로세스/아웃오브프로세스 패키지 경계를 유지합니다. 정확한 설치 호스트 검증 결과는 지원 범위와 분리해 기록하며, 해당 패키지가 실제로 통과한 호스트만 검증 완료로 명시합니다.

## 안전한 디버깅 동작

- 대입이 끝난 다음 줄에서 중단하십시오. 대입문 자체에 중단점을 두면 이전 값이나 null이 보일 수 있습니다.
- 포인터 기반 버퍼는 전달이 끝날 때까지 살아 있고 변경되지 않아야 합니다. 해제되었거나 접근할 수 없거나 일부만 읽히는 범위는 불완전한 이미지를 만들지 않고 오류로 중단합니다.
- 소스 주소는 메모리 위치이지 객체 식별자가 아닙니다. 같은 주소를 다시 열면 현재 바이트를 읽으며, 이 바이트는 재할당된 다른 객체의 것일 수도 있습니다.
- Continue 또는 프로세스 종료 후 기존 행은 마지막 렌더링 픽셀만 시각적 참고로 유지하며 이전 디버깅 프로세스의 메모리를 다시 읽지 않습니다.
