...
| Item | Verified state |
| --- | --- |
| Last verified | 2026-09-12 KST: the exact 2.0.9 package passed the reported `6768 x 3225` OpenCvSharp Mat, Emgu CV Mat, and ImagePtr workflow on VS2022 Community `17.9.34902.65` and serviced VS2022 Community `17.14.37516.0`; each opened 3 documents with 0 errors and changed live rows to `UNAVAILABLE` after exit. The same final bytes retained one pinned pane across registered Bitmap and RawBufferView opens on those hosts and stable VS2026 Community `18.9.12128.139`. Repeated identical manual errors refreshed one row at 540/900/1160 px. Aggregate and ten-version compatibility tests passed; all 71 installed files matched the VSIX on all three hosts. Official Gallery readback proved the public 2.0.8 package is byte-identical to the preserved final 2.0.8 candidate. Reported-size attempts on VS2026 18.9 and, after its update, 18.10 both stopped at the empty pre-debuggee start window and are not runtime passes. The public 2.0.8 in-place update also remains unexecuted. |
| Active repository | `C:\Git\RawBufferVisualizer_VSIX\RawBufferVisualizer_vs17.9_compat` |
| Branch / remote | The 2.0.9 release source is contained in the current `main` commit; its parent baseline is `368299b1a774`. Source commit/push was authorized on 2026-09-12; verify `origin/main` and CI from GitHub. No tag, Marketplace mutation, GitHub Release, or deployment is included. Repository: `https://github.com/Noah8218/RawBufferVisualizer.git`. |
| Implementation baseline | Public 2.0.8 contains the routed signed-Int32 payload and bounded Automatic Inspector. Current local 2.0.9 adds an 8 MiB checked-live-memory threshold for pointer-backed registered images, exact final-row ROI span calculation, stable technical debugger-RPC error fields, repeated-error row refresh, and permanent pinned-pane retention while disposing the temporary debugger handoff target. |
| Source and VSIX version | Local source metadata is `2.0.9` / `2.0.9.0`; public Marketplace is `2.0.8.0`. Exact local candidate: 2,520,780 bytes, SHA-256 `EB0F862EDA94FBDCC938A5AA26BF812C8A1BB1B25567FE8D7D1992A90799DDC0`. |
| Visual Studio support | Current source retains `[17.9,18.0)` and the 17.9 SDK/package boundary. The exact package passed its affected reported-size workflow on VS2022 `17.9.34902.65` and VS2022 `17.14.37516.0`, plus the pinned registered-open workflow on both and stable VS2026 `18.9.12128.139`. Final reported-size attempts remain unexecuted on both the earlier 18.9 installation and the current `18.10.12201.205` update because Visual Studio did not progress beyond its empty start window. |
| Public Marketplace version | Public `2.0.8.0`; the owner confirmed it is already deployed. Version `2.0.9.0` must be a new immutable package and is not yet published. |
| Git tags / GitHub Releases | Local annotated tag `v1.0.45` on `a23d8ad` created 2026-07-26; not pushed yet; no GitHub Release yet |
| Product stage | Public 2.0.8 has a field-reported medium native-image debugger-RPC timeout on VS2022 17.9. The frozen 2.0.9 candidate is qualified on the two VS2022 reported-size hosts and all three pinned-dock hosts. Existing 2.0.8 media remains representative because this correction does not change viewer layout or demonstrated controls. The exact public 2.0.8 package has been read back and identified, but staging it on the current 17.9 profile failed without replacing the valid 2.0.9 installation. The final stable-VS2026 reported-size run and actual public-2.0.8 in-place update remain unexecuted; the owner accepted those evidence gaps for source commit/push, not as test passes. CI and owner-controlled Marketplace upload remain separate. |
| Commit scope | The 2.0.9 release-source commit contains source, regression tests, installed-smoke updates, version metadata, README, English/Korean Marketplace copy, release notes, and durable incident rules. The owner explicitly authorized commit and `origin/main` push on 2026-09-12. |
...
## 2.0.9 Medium Native Transfer And ROI Correction

Status: Incomplete

Scope: Correct the public 2.0.8 debugger-RPC failure reported for medium pointer-backed OpenCvSharp Mat and ImagePtr values; apply the same path to Emgu CV; correct inferred ROI memory spans; replace localized remote-exception text in visible errors with technical RPC fields; coalesce repeated identical manual error rows; preserve a pinned docked Tool Window across registered visualizer opens; prepare version 2.0.9 release copy and VSIX without changing the viewer layout.

Acceptance criteria: inferred span ends at the final pixel row -> pass in protected-page tests for RawBufferView, ImagePtr, OpenCvSharp Mat, and Emgu CV Mat; reported `6768 x 3225` OpenCvSharp/Emgu/ImagePtr values -> pass with 3 documents/0 errors and live-to-`UNAVAILABLE` transition on final bytes for exact VS2022 17.9 and serviced VS2022 17.14, unexecuted on final bytes for stable VS2026 18.9; five OpenCvSharp and five Emgu compatibility points -> pass; visible RPC wording uses operation/source-or-chunk/byte-count/exception/HRESULT fields and preserves raw details only in the report -> pass; repeated identical manual failures refresh one row -> pass at 540/900/1160 px; sequential registered Bitmap/RawBufferView opens retain one pinned pane -> pass on all three hosts; exact package/install equality -> 71/71 on all three hosts.

Verification: routed final package under `candidate-final-r2`; aggregate self-tests under `final-selftest-build-r2`; ten-version legacy matrix under `legacy-matrix-build`; reported-size installed runs under `installed-vs2022-17.9-reported-size-r2-retry` and `installed-vs2022-17.14-reported-size-r2`; dock-retention installed runs under the three `*-dock-retention-r2*` roots; repeated-error width results under `ui-host-fix\after`; final 17.9 Release Announcement under `installed-vs2022-17.9-release-announcement-r2`; release communication, registration audit, Marketplace dry run under `marketplace\vs-publish-r2.json`, and 71/71 equality under `candidate-install-equality-r2.json`. Official 2.0.8 Gallery readback matched the preserved final candidate byte-for-byte; attempts to stage it on the current 17.9 profile failed before the in-place update could be exercised, and the existing 2.0.9 registration remained valid. The post-update VS2026 18.10 reported-size attempt is preserved under `installed-vs2026-18.10-reported-size-r2` and stopped before debuggee launch. All generated evidence is under `D:\OpenVisionLab-TestData\RawBufferVisualizer\release-2.0.9`.

Evidence: exact VSIX `D:\OpenVisionLab-TestData\RawBufferVisualizer\release-2.0.9\candidate-final-r2\RawBufferVisualizer-VisualStudioExtensibility-net472\RawBufferVisualizer.VisualStudio.Extensibility.vsix`; exact public 2.0.8 readback `D:\OpenVisionLab-TestData\RawBufferVisualizer\release-2.0.9\public-2.0.8-readback-20260912\RawBufferVisualizer-public-2.0.8.0.vsix`; qualification [release-qualification-2.0.9.md](release-qualification-2.0.9.md); user-provided `latest-error-report.txt` field report (local attachment, not committed); architecture regression rule in [ARCHITECTURE_AND_VALIDATION.md](ARCHITECTURE_AND_VALIDATION.md); tracked issue `.proofline/issues/PL-0002.json`; work contract `.proofline/work-contract-release-2.0.9.md`.

Boundary / next dependency: The source/build/package, VS2022 reported-size, three-host dock-retention, repeated-error, package-equality, and exact-public-2.0.8 identification scopes are qualified. CI for the pushed release-source commit remains the next repository gate. The final stable-VS2026 reported-size run and an in-place update from a profile already running exact public 2.0.8 remain unexecuted evidence gaps accepted by the owner for source delivery; Marketplace upload remains owner-controlled.
...
- Pointer-backed buffers at or above 8 MiB use checked direct process-memory reads instead of full-image debugger RPC snapshots. Buffers at or above 64 MiB still request a bounded sampled preview first.
...
Public 2.0.8 is the immutable Marketplace baseline. The 2.0.9 source and exact `candidate-final-r2` are frozen locally; no 2.0.9 Git action or external publication has occurred.

1. Complete the final stable-VS2026 reported-size run | Recommended model: `gpt-5.6-terra` | Reasoning effort: `medium`

   Reuse the already-installed exact `candidate-final-r2` bytes. Resolve only the Visual Studio 18.9 empty-start-window condition, then rerun the existing `6768 x 3225` OpenCvSharp/Emgu/ImagePtr smoke without rebuilding or repairing registration.

2. Exercise the in-place public 2.0.8 to exact 2.0.9 update | Recommended model: `gpt-5.6-terra` | Reasoning effort: `medium`

   Begin with the exact public Marketplace 2.0.8 package and update without uninstall, repair, or `/ResetSkipPkgs`; retain the resulting registration and installed-runtime evidence.

3. Commit and push 2.0.9 — prerequisite: explicit owner authorization after reviewing the exact VSIX and release copy | Recommended model: `gpt-5.6-luna` | Reasoning effort: `low`

   Tag, GitHub Release, Marketplace upload, and publication remain separate authorization boundaries.

Do not describe the failed Visual Studio 18.9 start attempts as product-runtime failures or as completed qualification.
...
Boundary / next dependency: Local implementation and exact-host evidence are complete, including the corrected direct `CV_32SC1` path on Visual Studio 2022 `17.9.34902.65`. The source/media history is pushed to `origin/main`, and integration-commit CI passed; no Marketplace upload, tag, release, or deployment was performed.
