...
- 이미 공개된 Marketplace `2.0.5.0`을 덮어쓰지 않고 패키지 버전을 `2.0.6.0`으로 올립니다.
- 검증된 2.0.5 동작과 비교해 기능 또는 호환성 변경은 없습니다.
- 실제 디버거 식 이름, 네이티브 `Ptr`과 `Pixels` 출처 구분, 이미지 배열 전체 등록, 실패 폐쇄형 포인터 읽기 및 음수 stride Bitmap 보정을 그대로 유지합니다.
...
`2.0.6` 패키지는 동일한 Marketplace ID, Visual Studio `17.9` SDK 하한 및 분리된 인프로세스/아웃오브프로세스 패키지 경계를 유지합니다. 정확한 설치 호스트 검증 결과는 지원 범위와 분리해 기록하며, 해당 패키지가 실제로 통과한 호스트만 검증 완료로 명시합니다.
