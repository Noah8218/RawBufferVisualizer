...
- Keep shipped and development claims explicit. Public `2.0.1.0` supports Visual Studio 2022 `17.14` or newer plus stable Visual Studio 2026 `18.x`; the exact, release-qualified but unpublished `2.0.2.0` candidate supports Visual Studio 2022 `17.9+` plus stable Visual Studio 2026 `18.x`, Community/Professional/Enterprise x64.
- Exact installed-VSIX versions and hosts actually tested belong in [docs/release-qualification-2.0.2.md](docs/release-qualification-2.0.2.md). Do not apply the unpublished `17.9+` claim to the Marketplace package until publication is explicitly authorized and public readback confirms the Marketplace payload.
- The VSIX manifest range `[17.14,18.0)` is the qualified 1.0.53/2.0.1 public contract; `[17.9,18.0)` is the qualified 2.0.2 release-candidate contract. Visual Studio 2026 uses the lower API-version bound for VSIX compatibility, supports Visual Studio API version 17.x, and ignores the upper bound. Any manifest, embedded release communication, or package-content change requires a new exact-package qualification.
