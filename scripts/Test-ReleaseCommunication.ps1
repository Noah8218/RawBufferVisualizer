...
Assert-Contains $overviewPath '(?m)^# Raw Buffer Visualizer\r?$' 'Marketplace overview product heading'
Assert-Contains $overviewPath "(?m)^## What's New In $escapedPackageVersion\r?$" 'Marketplace overview current-version section'
Assert-Contains $marketplaceNotesPath "(?m)^# Raw Buffer Visualizer $escapedPackageVersion\r?$" 'Marketplace release-notes heading'
Assert-Contains $embeddedNotesPath "(?m)^Raw Buffer Visualizer $escapedPackageVersion\r?$" 'Embedded VSIX release-notes heading'
