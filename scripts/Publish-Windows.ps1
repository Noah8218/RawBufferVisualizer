[CmdletBinding()]
param(
    [ValidateSet('net8.0-windows', 'net472')]
...
try {
    ...
    if (-not $SkipSamples) {
        & dotnet run --project (Join-Path $repoRoot 'samples\RawBufferVisualizer.Samples\RawBufferVisualizer.Samples.csproj') --configuration Release --framework net8.0
        if ($LASTEXITCODE -ne 0) {
